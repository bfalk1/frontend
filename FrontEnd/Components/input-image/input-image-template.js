...
export const inputImageTemplate = (context) => {
    return html `
    <style>
    .file-label {
        cursor: pointer;
    ...
    <div id="drop-zone" class="drop-zone">
        ${context.selectedFile.length === 0 ? html`
        ...
        <input type="file" id="file-input" multiple style="display: none;" @change=${(e)=>context.handleFileChange(e)}>
        <label for="file-input" id="file-label" class="file-label">Upload Files</label>`
        :html``}
        ${context.selectedFile.length > 0 
            ? html`<img src="${(context.selectedFile)}" alt="Selected Image" @click=${(e) => context.viewImageSize(e)}/>
            `: html``}
    </div>
     `  
}